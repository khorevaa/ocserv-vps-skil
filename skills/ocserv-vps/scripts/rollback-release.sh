...
[[ -n "${HOST}" && -n "${TO_VERSION}" ]] || { printf '%s\n' '--host and --to-version are required.' >&2; exit 2; }
[[ "${TO_VERSION}" == "previous" ]] || ocserv_validate_version "${TO_VERSION}"
ocserv_validate_port 'SSH port' "${SSH_PORT}"
[[ "${HEALTH_TIMEOUT}" =~ ^[0-9]+$ ]] && (( HEALTH_TIMEOUT >= 15 && HEALTH_TIMEOUT <= 300 )) || {
  printf '%s\n' '--health-timeout must be from 15 to 300.' >&2
  exit 2
}
...
ocserv_run_remote \
  ...
  --to-version "${TO_VERSION}" --health-timeout "${HEALTH_TIMEOUT}" --approve-restart
