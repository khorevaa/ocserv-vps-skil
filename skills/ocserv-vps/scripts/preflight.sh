...
usage() {
  cat <<'EOF'
Usage:
  preflight.sh --host <ssh_target> [options]

Options:
  --target-version <version>     Report whether the release path is free.
...
while [[ $# -gt 0 ]]; do
  case "$1" in
    ...
    --target-version) ocserv_require_value "$1" "${2:-}"; TARGET_VERSION="$2"; shift 2 ;;
...
remote_args=(--config "${CONFIG}")
if [[ -n "${TARGET_VERSION}" ]]; then
  remote_args+=(--target-version "${TARGET_VERSION}")
fi
